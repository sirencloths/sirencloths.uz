...
export default function ProductDetailClient({ id, title, price, currencyCode, image, variants, initialColor, sizeGuideImageUrl }: Props) {
  ...
  const requestedSize = searchParams.get("size")?.trim().toLocaleLowerCase("uz-UZ") ?? "";
  useEffect(() => {
    if (!requestedSize) return;
    const matchingSize = sizes.find((size) => size.key === requestedSize);
    if (matchingSize) setSelectedSize(matchingSize.key);
  }, [requestedSize, sizes]);
  ...
  const numberLocale = locale === "ru" ? "ru-RU" : locale === "en" ? "en-GB" : "uz-UZ";
  const displayPrice = selectedVariant?.price ? `${new Intl.NumberFormat(numberLocale, { maximumFractionDigits: 0 }).format(Number(selectedVariant.price)).replace(/\u00A0/g, ".")} ${currencyCode === "UZS" ? "UZS" : currencyCode}` : price;
  const oldPrice = selectedVariant?.originalPrice ? `${new Intl.NumberFormat(numberLocale, { maximumFractionDigits: 0 }).format(Number(selectedVariant.originalPrice)).replace(/\u00A0/g, ".")} ${currencyCode === "UZS" ? "UZS" : currencyCode}` : "";

  return <>
    <p className="product-detail-sku">SKU: {selectedVariant?.sku || "—"}</p>
    <div className="product-detail-pricing"><p className={`product-detail-price${selectedVariant?.discountPercent ? " is-sale" : ""}`}>{displayPrice}</p>{oldPrice && <del>{oldPrice}</del>}{selectedVariant?.discountPercent ? <div className="product-detail-sale-meta"><span>−{selectedVariant.discountPercent}%</span>{remaining && <time aria-label="Chegirma tugash vaqti">{remaining}</time>}</div> : null}</div>
    ...
    {!available && <p className="product-availability is-unavailable">{t("soldOut")}</p>}
    <div className="product-actions"><AddToCartButton id={selectedVariant?.id ?? id} productId={id} title={title} price={selectedVariant?.price || price} image={selectedImage} color={selectedColorLabel} size={selectedSizeOption?.label ?? "ONE SIZE"} isSale={Boolean(selectedVariant?.discountPercent)} inventoryQuantity={selectedVariant?.inventoryQuantity} /><button type="button" className={`product-favorite-btn${isFavorite(favoriteProduct) ? " is-liked" : ""}`} aria-label={t("favoriteAdd")} aria-pressed={isFavorite(favoriteProduct)} onClick={() => toggleFavorite(favoriteProduct)}><Image src={isFavorite(favoriteProduct) ? "/icons/heart-filled.svg" : "/icons/heart.svg"} alt="" width={28} height={28} /></button></div>
    {isSizeGuideOpen && sizeGuideImageUrl && <div className="size-guide-modal-backdrop" role="presentation" onMouseDown={() => setIsSizeGuideOpen(false)}><section className="size-guide-modal" role="dialog" aria-modal="true" aria-label={t("sizeHelp")} onMouseDown={(event) => event.stopPropagation()}><header><div><p>{t("sizeGuide")}</p><h2>{t("sizeHelp")}</h2></div><button type="button" onClick={() => setIsSizeGuideOpen(false)} aria-label={t("close")}>×</button></header><img src={sizeGuideImageUrl} alt={t("sizeHelp")} /></section></div>}
  </>;
}
