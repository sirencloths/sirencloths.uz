        ...
        <TabsContent value="catalog">
          <div className="admin-card-grid">
            ...
            <Taxonomy
              ...
              rows={collections}
              submit={(e) => taxonomy("collections", e)}
            />
          </div>
        </TabsContent>
...
function CategoryManager({ categories, products, token, onChanged }: { categories: Taxonomy[]; products: Product[]; token: string; onChanged: () => Promise<void> }) {
  ...
  if (view === "list") return <Card className="category-manager category-manager--list"><CardHeader><div><p className="ui-overline">KATEGORIYALAR</p><CardTitle>Kategoriyalar ro‘yxati</CardTitle></div><Button onClick={() => open()}><Plus size={16} />Yangi</Button></CardHeader><CardContent><div className="category-row-list">{categories.map((category) => <article key={category.id}><b>{category.name}</b><span>{category.productCount ?? products.filter((product) => product.categoryId === category.id).length} mahsulot</span><Button size="sm" onClick={() => open(category)}>Edit</Button><Button size="sm" variant="outline" onClick={() => open(category, true)}>Mahsulotlar</Button></article>)}{!categories.length && <Empty>Kategoriya yo‘q.</Empty>}</div></CardContent></Card>;
  if (!editing) return null;
  return <Card className="category-manager category-manager--editor"><CardHeader><div><p className="ui-overline">KATEGORIYALAR / {editing.id ? "EDIT" : "YANGI"}</p><CardTitle>{editing.id ? editing.name || "Kategoriyani tahrirlash" : "Yangi kategoriya"}</CardTitle><CardDescription>Saqlangach kategoriya ro‘yxatiga qaytasiz.</CardDescription></div><Button variant="outline" onClick={() => { setView("list"); setEditing(null); }}>← Ro‘yxat</Button></CardHeader><CardContent><section className="category-editor"><header><div><h3>Asosiy ma’lumotlar</h3></div><Button size="sm" onClick={() => void saveCategory()} disabled={busy}>{busy ? "Saqlanmoqda…" : "Saqlash"}</Button></header><div className="ui-form-grid"><Field label="Nomi"><input value={editing.name} onChange={(event) => setEditing({ ...editing, name: event.target.value })} required /></Field><Field label="Slug"><input value={editing.slug} onChange={(event) => setEditing({ ...editing, slug: event.target.value })} required /></Field></div><label className="admin-record-active"><input type="checkbox" checked={editing.isVisible} onChange={(event) => setEditing({ ...editing, isVisible: event.target.checked })} /> Saytda ko‘rsatish</label>{editing.id && <><div className="category-product-head"><div><h3>Mahsulotlar</h3><span>✓ — shu kategoriya; boshqa mahsulotni bosish uni shu yerga ko‘chiradi.</span></div><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Mahsulot qidirish..." /></div><div className="category-product-list">{visible.map((product) => { const assigned = product.categoryId === editing.id; const owner = categories.find((category) => category.id === product.categoryId); return <button type="button" key={product.id} className={assigned ? "is-assigned" : ""} disabled={busy} onClick={() => !assigned && void move(product)}><span>{assigned ? "✓" : "+"}</span><div><b>{product.title}</b><small>{assigned ? "Shu kategoriya" : owner ? `Hozir: ${owner.name}` : "Kategoriya biriktirilmagan"}</small></div><em>{assigned ? "Biriktirilgan" : "Shu yerga ko‘chirish"}</em></button>; })}{!visible.length && <Empty>Mahsulot topilmadi.</Empty>}</div></>}<AdminToast message={message} /></section></CardContent></Card>;
}
