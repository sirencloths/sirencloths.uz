...
function NotificationManager({ items, token, onChanged }: { items: StoreNotification[]; token: string; onChanged: (items: StoreNotification[]) => void }) {
  ...
  const create = async (event: FormEvent) => {
    ...
    const item: StoreNotification = { id: crypto.randomUUID(), kind: draft.kind, title: draft.title, text: draft.text, imageUrl: draft.imageUrl.trim(), href: draft.href.trim() || "/", createdAt: new Date().toISOString(), isActive: draft.isActive, clicks: 0, clickVisitorIds: [] };
