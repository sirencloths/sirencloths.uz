...
import { UnitsHistoryChart } from "./UnitsHistoryChart";
import {
  Archive,
  ArrowDown,
  ArrowUp,
...
const adminTabs: Tab[] = ["dashboard", "products", "catalog", "orders", "refunds", "delivery", "pickup", "customers", "discounts", "partners", "finance", "report_online", "report_offline", "report_combined", "business_cost", "history_cost", "history_sales", "history_units", "payments", "currencies", "analytics", "analytics_sales", "analytics_products", "analytics_drops", "analytics_inventory", "analytics_customers", "analytics_traffic", "analytics_marketing", "analytics_geography", "content", "notifications", "pages", "team", "audit", "settings", "offline_cashier", "offline_inventory", "offline_sales", "offline_reports"];
...
type RefundActor = "customer" | "operator" | "service";
type RefundNote = { cancellation?: { reason?: string; evidenceUrl?: string | null; refundMethod?: string; refundState?: string; refundActor?: RefundActor; requestedAt?: string } };
function refundNote(order: Order): RefundNote { try { return JSON.parse(order.note || "{}"); } catch { return {}; } }
function refundActor(order: Order): RefundActor {
  const note = refundNote(order).cancellation;
  if (note?.refundActor === "service" || paymeNote(order).payme?.cancelTime) return "service";
  if (order.status === "refunded" || order.paymentStatus === "refunded") return "customer";
  if (note?.refundActor) return note.refundActor;
  return "operator";
}
function RefundActorBadge({ actor }: { actor: RefundActor }) {
  const label: Record<RefundActor, string> = { customer: "Mijoz refundi", operator: "Bajaruvchi refundi", service: "Servis refundi" };
  const Icon = actor === "customer" ? Users : actor === "operator" ? ShieldCheck : CreditCard;
  return <span className={`refund-actor is-${actor}`}><Icon size={14} />{label[actor]}</span>;
}
type PaymeNote = { payme?: { transactionId?: string; state?: number; createTime?: number; performTime?: number; cancelTime?: number } };
function paymeNote(order: Order): PaymeNote { try { return JSON.parse(order.note || "{}"); } catch { return {}; } }
function RefundManager({ orders, onRefresh }: { orders: Order[]; onRefresh: () => void }) {
  const [query, setQuery] = useState(""); const [channel, setChannel] = useState<"all" | "pickup" | "delivery">("all"); const [method, setMethod] = useState<"all" | "cash" | "payme">("all"); const [actor, setActor] = useState<"all" | RefundActor>("all"); const [selected, setSelected] = useState<Order | null>(null);
  const refunded = orders.filter((order) => order.status === "refunded" || order.paymentStatus === "refunded" || (order.status === "cancelled" && order.paymentStatus === "not_paid"));
  const visible = refunded.filter((order) => { const note = refundNote(order).cancellation; const pickup = order.shippingAddress?.fulfillmentMethod === "pickup"; const cash = note?.refundMethod === "cash" || /naqd|nalich|cash|налич/i.test(order.paymentMethod || ""); const haystack = `${order.orderNumber} ${order.customer?.firstName || ""} ${order.customer?.lastName || ""} ${order.customer?.email || ""} ${order.customer?.phone || ""} ${order.shippingAddress?.pickupLocationName || ""}`.toLowerCase(); return haystack.includes(query.toLowerCase()) && (channel === "all" || (channel === "pickup") === pickup) && (method === "all" || (method === "cash") === cash) && (actor === "all" || refundActor(order) === actor); });
  const paidRefunds = refunded.filter((order) => order.status === "refunded" || order.paymentStatus === "refunded"); const total = paidRefunds.reduce((sum, order) => sum + Number(order.totalAmount || 0), 0); const cashCount = paidRefunds.filter((order) => /naqd|nalich|cash|налич/i.test(order.paymentMethod || "")).length;
  return <section className="refund-manager"><header className="tailadmin-page-heading"><div><p className="ui-overline">REFUND MARKAZI</p><h2>Qaytarilgan to‘lovlar</h2><span>Refund cheklari, to‘lovsiz bekor qilingan buyurtmalar va qaytarish manbasi bitta joyda.</span></div><Button type="button" variant="outline" onClick={onRefresh}><RefreshCw size={16} /> Yangilash</Button></header><div className="refund-summary"><article><span>Jami refund</span><b>{money(total, "UZS")}</b><small>{paidRefunds.length} ta to‘langan buyurtma</small></article><article><span>Naqd qaytarilgan</span><b>{cashCount} ta</b><small>Olib ketish nuqtasida</small></article><article><span>Payme refund</span><b>{paidRefunds.length - cashCount} ta</b><small>Onlayn to‘lovdan</small></article></div><div className="tailadmin-data-card refund-data-card"><div className="refund-toolbar"><label><Search size={18} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Chek, mijoz yoki telefon qidirish..." /></label><select value={channel} onChange={(event) => setChannel(event.target.value as typeof channel)}><option value="all">Barcha manbalar</option><option value="pickup">Olib ketish nuqtasi</option><option value="delivery">Kuryer yetkazib berish</option></select><select value={method} onChange={(event) => setMethod(event.target.value as typeof method)}><option value="all">Barcha to‘lovlar</option><option value="cash">Naqd pul</option><option value="payme">Payme</option></select><select value={actor} onChange={(event) => setActor(event.target.value as typeof actor)}><option value="all">Barcha tomonlar</option><option value="customer">Mijoz</option><option value="operator">Bajaruvchi</option><option value="service">Servis / hamkor</option></select></div><div className="admin-module-table-wrap"><table className="admin-module-table refund-table"><thead><tr><th>Chek</th><th>Mijoz</th><th>Refund kimdan</th><th>Refund manbasi</th><th>To‘lov</th><th>Summa</th><th>Qaytarilgan sana</th><th></th></tr></thead><tbody>{visible.map((order) => { const info = refundNote(order).cancellation; const pickup = order.shippingAddress?.fulfillmentMethod === "pickup"; const cash = info?.refundMethod === "cash" || /naqd|nalich|cash|налич/i.test(order.paymentMethod || ""); const unpaid = order.paymentStatus === "not_paid"; return <tr key={order.id}><td><b>#{order.orderNumber}</b><small>{unpaid ? "To‘lovsiz bekor qilingan" : "Refund chek"}</small></td><td><b>{`${order.customer?.firstName || ""} ${order.customer?.lastName || ""}`.trim() || order.customer?.email || "Mijoz"}</b><small>{order.customer?.phone || order.customer?.email || "—"}</small></td><td><RefundActorBadge actor={refundActor(order)} /></td><td><span className="refund-source">{pickup ? <MapPin size={15} /> : <Truck size={15} />}{pickup ? order.shippingAddress?.pickupLocationName || "Olib ketish nuqtasi" : "Kuryer orqali"}</span></td><td><span className={`refund-method ${unpaid ? "is-unpaid" : cash ? "is-cash" : "is-payme"}`}>{unpaid ? "To‘lov olinmagan" : cash ? "Naqd qaytarildi" : "Payme refund"}</span></td><td><b className={unpaid ? "refund-money is-unpaid" : "refund-money"}>{unpaid ? "—" : `−${money(Number(order.totalAmount), "UZS")}`}</b></td><td>{readableDate(info?.requestedAt || order.createdAt)}<small>{unpaid ? "Buyurtma bekor qilindi" : "Refund yakunlandi"}</small></td><td><button type="button" className="refund-open" onClick={() => setSelected(order)}><Eye size={16} /> Ko‘rish</button></td></tr>; })}{!visible.length && <tr><td colSpan={8}><Empty>{query ? "Qidiruv bo‘yicha refund topilmadi." : "Hali refund yoki to‘lovsiz bekor qilish yo‘q."}</Empty></td></tr>}</tbody></table></div></div>{selected && <RefundDetailDialog order={selected} onClose={() => setSelected(null)} />}</section>;
}
...
function CancellationDialog({ order, token, onClose, onDone }: { order: Order; token: string; onClose: () => void; onDone: (message: string) => void }) {
  ...
  const cash = /nalich|naqd|cash|налич/i.test(order.paymentMethod ?? ""); const paid = order.paymentStatus === "paid";
  const submit = async (event: FormEvent) => { event.preventDefault(); if (paid && !reason.trim()) return; setSaving(true); try { let evidenceUrl: string | undefined; if (photo) { const data = new FormData(); data.append("file", photo); evidenceUrl = (await api<{ url: string }>("/admin/content/banners/upload", token, { method: "POST", body: data })).url; } await api(`/admin/orders/${order.id}/cancel`, token, { method: "POST", body: JSON.stringify({ reason: reason.trim(), evidenceUrl }) }); onDone(!paid ? "Buyurtma bekor qilindi." : cash ? "Buyurtma bekor qilindi. Naqd pulni mijozga qaytaring." : "Buyurtma bekor qilindi. Payme refund qayta ishlashga yuborildi."); } catch (error) { onDone(error instanceof Error ? error.message : "Buyurtmani bekor qilib bo‘lmadi."); } finally { setSaving(false); } };
  return <div className="inventory-confirm-backdrop cancellation-backdrop"><form className="inventory-confirm cancellation-dialog" onSubmit={submit}><button className="reference-modal-close" type="button" onClick={onClose}>×</button><p className="ui-overline">BUYURTMANI BEKOR QILISH</p><h3>Chek #{order.orderNumber}</h3><p>{!paid ? "Bu buyurtma hali to‘lanmagan. Refund amalga oshirilmaydi." : cash ? "Naqd to‘lov mijozga naqd qaytariladi." : "Payme to‘lovi refund jarayoniga yuboriladi."}</p><Field label={paid ? "Bekor qilish sababi" : "Izoh (ixtiyoriy)"}><textarea value={reason} onChange={(event) => setReason(event.target.value)} placeholder="Sababni yozing" required={paid} /></Field><Field label="Foto dalil (ixtiyoriy)"><input type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => setPhoto(event.target.files?.[0] ?? null)} /><small>{photo?.name || "Foto yuklanmagan"}</small></Field><div className="inventory-confirm-actions"><Button type="button" variant="outline" onClick={onClose}>Ortga</Button><Button disabled={saving}>{saving ? "Saqlanmoqda…" : paid ? "Bekor qilish va refund" : "Buyurtmani bekor qilish"}</Button></div></form></div>;
}
...
type PaymeAdminStatus = { configured: boolean; merchantConfigured: boolean; keyConfigured: boolean; mode: "test" | "production"; callbackUrl: string; returnUrl: string };
function PaymeIntegrationPanel({ token, onNotify }: { token: string; onNotify: (message: string) => void }) {
  const [status, setStatus] = useState<PaymeAdminStatus | null>(null); const [error, setError] = useState("");
  const load = useCallback(async () => { try { setError(""); setStatus(await api<PaymeAdminStatus>("/admin/payments/payme/status", token)); } catch (reason) { setError(reason instanceof Error ? reason.message : "Payme holati yuklanmadi."); } }, [token]);
  useEffect(() => { void load(); }, [load]);
  const copy = async (value: string, label: string) => { try { await navigator.clipboard.writeText(value); onNotify(`${label} nusxalandi.`); } catch { onNotify("Nusxalash uchun URLni belgilang."); } };
  return <section className="payme-integration-panel"><header><div><p className="ui-overline">PAYME INTEGRATSIYASI</p><h3>Onlayn to‘lov ulanishi</h3><span>Kalitlar faqat server environment’da saqlanadi; admin panel ularni hech qachon ko‘rsatmaydi.</span></div><Button type="button" size="sm" variant="outline" onClick={() => void load()}><RefreshCw size={15} /> Yangilash</Button></header>{error ? <p className="admin-module-state is-error">{error}</p> : <><div className="payme-integration-status"><span className={status?.configured ? "is-ready" : "is-pending"}>{status?.configured ? "ULANGAN" : "SOZLASH KUTILMOQDA"}</span><b>{status?.mode === "production" ? "Production" : "Test rejimi"}</b></div><div className="payme-integration-checks"><span className={status?.merchantConfigured ? "is-done" : ""}>Merchant ID {status?.merchantConfigured ? "tayyor" : "kiritilmagan"}</span><span className={status?.keyConfigured ? "is-done" : ""}>Merchant key {status?.keyConfigured ? "tayyor" : "kiritilmagan"}</span><span>Callback himoyasi: Basic Auth</span></div><div className="payme-integration-url"><span>PAYME CALLBACK URL</span><code>{status?.callbackUrl || "Yuklanmoqda…"}</code><Button type="button" size="sm" variant="outline" disabled={!status?.callbackUrl} onClick={() => void copy(status!.callbackUrl, "Callback URL")}>Nusxalash</Button></div><div className="payme-integration-url"><span>MIJOZ QAYTISH URL</span><code>{status?.returnUrl || "Yuklanmoqda…"}</code><Button type="button" size="sm" variant="outline" disabled={!status?.returnUrl} onClick={() => void copy(status!.returnUrl, "Qaytish URL")}>Nusxalash</Button></div><p className="payme-integration-hint">Keyingi qadam: Payme Business kabinetiga callback URLni kiriting, keyin test buyurtma bilan to‘lov va callbackni tekshiring.</p></>}</section>;
}
type FinanceOrderDetail = { id: string; orderNumber: string; customerName: string; phone?: string | null; paymentMethod?: string | null; paymentStatus?: string | null; fulfillmentStatus?: string | null; items: Array<{ title: string; sku?: string | null; quantity: number; total: number }> };
type FinanceTransaction = { id: string; source: string; type: string; title: string; category: string; amount: number; currencyCode: string; occurredAt: string; note?: string | null; receiptUrl?: string | null; order?: FinanceOrderDetail };
type FinanceOverviewData = { summary: { grossRevenue: number; onlineRevenue: number; offlineRevenue: number; refunds: number; manualIncome: number; expenses: number; offlineExpenses: number; netRevenue: number; netProfit: number; paidOrders: number }; transactions: FinanceTransaction[] };
function CostHistorySection({ products }: { products: Product[] }) { const today = new Date().toISOString().slice(0, 10); const [from, setFrom] = useState(""); const [to, setTo] = useState(today); const events = products.flatMap((product) => product.variants.map((variant) => ({ date: (product.createdAt || today).slice(0, 10), value: amountOf(variant.attributes?.costPrice) * Math.max(0, variant.totalInventoryAdded ?? variant.inventoryQuantity + offlineOf(variant)) }))).filter((item) => item.value > 0).sort((a, b) => a.date.localeCompare(b.date)); const total = events.reduce((sum, item) => sum + item.value, 0); const start = from || events[0]?.date || today; let running = events.filter((item) => item.date < start).reduce((sum, item) => sum + item.value, 0); const grouped = events.filter((item) => item.date >= start && item.date <= to).reduce<Record<string, number>>((all, item) => { all[item.date] = (all[item.date] || 0) + item.value; return all; }, {}); const points = Object.entries(grouped).map(([date, value]) => { running += value; return { date, value: running }; }); if (!points.length) points.push({ date: start, value: running }); const max = Math.max(1, ...points.map((point) => point.value)); const path = points.map((point, index) => `${index ? "L" : "M"}${points.length === 1 ? 40 : 40 + index * 880 / (points.length - 1)},${300 - point.value / max * 230}`).join(" "); return <section className="business-cost-page cost-history-page"><header className="tailadmin-page-heading"><div><p className="ui-overline">HISTORY</p><h2>Asl narx tarixi</h2><span>Biznes ochilganidan buyon tovarlar kiritilgan tannarxning umumiy dinamikasi.</span></div><div className="finance-filters"><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} aria-label="Dan" /><input type="date" value={to} onChange={(event) => setTo(event.target.value)} aria-label="Gacha" /></div></header><div className="business-cost-grid"><article className="is-total"><span>0 dan hozirgacha umumiy asl narx</span><b>{money(total, "UZS")}</b><small>{events.length} ta variant kiritilishi</small></article><article><span>Tanlangan davrda qo‘shilgan</span><b>{money(events.filter((item) => item.date >= start && item.date <= to).reduce((sum, item) => sum + item.value, 0), "UZS")}</b><small>{start} — {to}</small></article><article><span>Oxirgi nuqtadagi jami</span><b>{money(points.at(-1)?.value || 0, "UZS")}</b><small>Tanlangan davr yakuni</small></article></div><section className="cost-history-chart"><header><div><h3>Asl narx dinamikasi</h3><span>Line chart · tovarlar kiritilgan sana bo‘yicha</span></div></header><div className="cost-history-plot"><svg viewBox="0 0 960 340" role="img" aria-label="Asl narx dinamikasi"><line x1="40" y1="300" x2="920" y2="300" /><line x1="40" y1="70" x2="920" y2="70" /><path d={path} /><path className="cost-history-area" d={`${path} L920,300 L40,300 Z`} /></svg></div><footer><span>{points[0]?.date}</span><b>{money(max, "UZS")}</b><span>{points.at(-1)?.date}</span></footer></section></section>; }
type BusinessReportData = { period: { from: string; to: string }; online: Record<string, number>; offline: Record<string, number>; combined: Record<string, number>; paymentBreakdown: Array<{ method: string; count: number; amount: number }>; productBreakdown: Array<{ title: string; sku?: string | null; quantity: number; revenue: number }>; statusBreakdown: Record<string, number>; transactions: Array<{ id: string; source?: string; number: string; date: string; payment?: string | null; amount: number; discount?: number; cashier?: string }> };
function BusinessCostOverview({ products }: { products: Product[] }) { const rows = products.flatMap((product) => product.variants.map((variant) => ({ product: product.title, sku: variant.sku, cost: amountOf(variant.attributes?.costPrice), online: Math.max(0, variant.inventoryQuantity), offline: offlineOf(variant) }))).filter((row) => row.online + row.offline > 0); const online = rows.reduce((sum, row) => sum + row.cost * row.online, 0); const offline = rows.reduce((sum, row) => sum + row.cost * row.offline, 0); const missing = rows.filter((row) => row.cost <= 0); return <section className="business-cost-page"><header className="tailadmin-page-heading"><div><p className="ui-overline">BUSINESS CAPITAL</p><h2>Biznesning asl narxi</h2><span>Omborda qolgan har bir variant tannarxi bo‘yicha hisoblangan kapital.</span></div></header><div className="business-cost-grid"><article><span>Online tovarlar tannarxi</span><b>{money(online, "UZS")}</b><small>{rows.reduce((sum, row) => sum + row.online, 0)} ta qoldiq</small></article><article><span>Offline tovarlar tannarxi</span><b>{money(offline, "UZS")}</b><small>{rows.reduce((sum, row) => sum + row.offline, 0)} ta qoldiq</small></article><article className="is-total"><span>Umumiy biznes kapitali</span><b>{money(online + offline, "UZS")}</b><small>Online + Offline</small></article></div><section className="finance-journal"><header><div><h3>Tannarx kesimi</h3><span>{missing.length ? `${missing.length} ta variantda tannarx kiritilmagan` : "Barcha qoldiq tannarx bilan hisoblangan"}</span></div></header><div className="finance-table-wrap"><table><thead><tr><th>Mahsulot</th><th>SKU</th><th>Tannarx</th><th>Online</th><th>Offline</th><th>Jami kapital</th></tr></thead><tbody>{rows.map((row) => <tr key={row.sku}><td>{row.product}</td><td>{row.sku}</td><td>{row.cost ? money(row.cost, "UZS") : "Kiritilmagan"}</td><td>{row.online} ta</td><td>{row.offline} ta</td><td>{money(row.cost * (row.online + row.offline), "UZS")}</td></tr>)}</tbody></table></div></section></section>; }
function BusinessReport({ scope, token }: { scope: "online" | "offline" | "combined"; token: string }) {
  const [data, setData] = useState<BusinessReportData | null>(null); const [from, setFrom] = useState(() => new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString().slice(0, 10)); const [to, setTo] = useState(() => new Date().toISOString().slice(0, 10)); const [loading, setLoading] = useState(false);
  const load = useCallback(async () => { setLoading(true); try { setData(await api<BusinessReportData>(`/admin/finance/reports?scope=${scope}&from=${from}&to=${to}`, token)); } finally { setLoading(false); } }, [scope, from, to, token]);
  useEffect(() => { void load(); }, [load]);
  const title = scope === "online" ? "Online hisobot" : scope === "offline" ? "Offline hisobot" : "Umumiy hisobot"; const summary = data?.[scope] ?? {};
  const cards = scope === "online" ? [["Savdo", summary.revenue], ["Refund", summary.refunds], ["Chegirma", summary.discounts], ["Sof tushum", summary.net], ["To‘langan", summary.paid], ["O‘rtacha chek", summary.averageOrder]] : scope === "offline" ? [["Savdo", summary.revenue], ["Xarajat", summary.expenses], ["Chegirma", summary.discounts], ["Sof tushum", summary.net], ["Cheklar", summary.receipts], ["O‘rtacha chek", summary.averageReceipt]] : [["Jami savdo", summary.revenue], ["Refund", summary.refunds], ["Xarajat", summary.expenses], ["Chegirma", summary.discounts], ["Sof tushum", summary.net], ["Jami savdolar", summary.transactions]];
  return <section className="finance-manager"><header className="tailadmin-page-heading"><div><p className="ui-overline">HISOBOT</p><h2>{title}</h2><span>Davr, to‘lov, mahsulot, status va har bir operatsiya kesimida batafsil moliyaviy natija.</span></div><div className="finance-filters"><input type="date" value={from} onChange={(event) => setFrom(event.target.value)} /><input type="date" value={to} onChange={(event) => setTo(event.target.value)} /><Button type="button" onClick={() => void load()} disabled={loading}>{loading ? "Yuklanmoqda…" : "Yangilash"}</Button></div></header><div className="finance-summary-grid">{cards.map(([label, value]) => <article key={String(label)}><span>{label}</span><b>{typeof value === "number" && String(label).includes("chek") || String(label).includes("To‘langan") || String(label).includes("Cheklar") || String(label).includes("savdolar") ? `${value ?? 0} ta` : money(Number(value ?? 0), "UZS")}</b></article>)}</div><div className="finance-journal"><header><div><h3>To‘lov va statuslar kesimi</h3><span>Tanlangan davr: {data?.period.from} — {data?.period.to}</span></div></header><div className="finance-payment-breakdown">{data?.paymentBreakdown.map((item) => <p key={item.method}><b>{item.method}</b><span>{item.count} ta · {money(item.amount, "UZS")}</span></p>)}</div><div className="finance-status-breakdown">{Object.entries(data?.statusBreakdown ?? {}).map(([key, value]) => <span key={key}>{key}: <b>{value}</b></span>)}</div></div><div className="finance-journal"><header><h3>Top mahsulotlar</h3></header><div className="finance-table-wrap"><table><thead><tr><th>Mahsulot</th><th>SKU</th><th>Sotilgan</th><th>Tushum</th></tr></thead><tbody>{data?.productBreakdown.map((item) => <tr key={`${item.title}-${item.sku}`}><td>{item.title}</td><td>{item.sku || "—"}</td><td>{item.quantity} ta</td><td>{money(item.revenue, "UZS")}</td></tr>)}</tbody></table></div></div><div className="finance-journal"><header><h3>Operatsiyalar</h3></header><div className="finance-table-wrap"><table><thead><tr><th>Manba</th><th>Chek</th><th>Sana</th><th>To‘lov</th><th>Chegirma</th><th>Summa</th></tr></thead><tbody>{data?.transactions.map((item) => <tr key={item.id}><td>{item.source || (scope === "online" ? "Online" : "Offline")}</td><td>{item.number}</td><td>{new Intl.DateTimeFormat("uz-UZ", { dateStyle: "medium", timeStyle: "short" }).format(new Date(item.date))}</td><td>{item.payment || "—"}</td><td>{money(item.discount || 0, "UZS")}</td><td>{money(item.amount, "UZS")}</td></tr>)}</tbody></table></div></div></section>;
}
function FinanceManager({ token, onNotice }: { token: string; onNotice: (message: string) => void }) {
  const [data, setData] = useState<FinanceOverviewData | null>(null); const [loading, setLoading] = useState(false); const [error, setError] = useState(""); const [filter, setFilter] = useState<"all" | "income" | "expense" | "refund">("all"); const [showForm, setShowForm] = useState(false); const [selected, setSelected] = useState<FinanceTransaction | null>(null); const [draft, setDraft] = useState({ type: "expense", title: "", category: "", amount: "", note: "", occurredAt: new Date().toISOString().slice(0, 10) });
  const load = useCallback(async () => { if (!token) return; setLoading(true); setError(""); try { const [overview, orders] = await Promise.all([api<FinanceOverviewData>("/admin/finance", token), api<Order[]>("/admin/orders", token)]); const byNumber = new Map(orders.map((order) => [String(order.orderNumber), order])); overview.transactions = overview.transactions.map((item) => { const number = item.title.match(/#(\d+)/)?.[1]; const order = number ? byNumber.get(number) : undefined; if (!order || !["order", "refund"].includes(item.source)) return item; return { ...item, order: { id: order.id, orderNumber: order.orderNumber, customerName: `${order.customer?.firstName || ""} ${order.customer?.lastName || ""}`.trim() || order.customer?.email || "Mijoz", phone: order.customer?.phone, paymentMethod: order.paymentMethod, paymentStatus: order.paymentStatus, fulfillmentStatus: order.fulfillmentStatus, items: (order.items ?? []).map((line) => ({ title: line.titleSnapshot, sku: line.skuSnapshot, quantity: line.quantity, total: Number(line.unitPrice) * line.quantity })) } }; }); setData(overview); } catch (reason) { setError(reason instanceof Error ? reason.message : "Moliyaviy ma’lumotlar yuklanmadi."); } finally { setLoading(false); } }, [token]);
  useEffect(() => { void load(); }, [load]);
  ...
  return <section className="finance-manager"><header className="tailadmin-page-heading"><div><p className="ui-overline">MOLIYA</p><h2>Daromad va xarajatlar</h2><span>Onlayn do‘kon, offline kassa, refund va hisobot xarajatlari bitta jurnalga jamlanadi.</span></div><Button type="button" onClick={() => setShowForm(true)}><Plus size={16} /> Operatsiya qo‘shish</Button></header>{error && <p className="team-management-error">{error}</p>}{loading && !data && <p className="team-management-loading">Yuklanmoqda…</p>}<div className="finance-summary-grid">{[["Jami savdo", summary?.grossRevenue ?? 0, "income"], ["Refund", summary?.refunds ?? 0, "refund"], ["Xarajat", summary?.expenses ?? 0, "expense"], ["Sof foyda", summary?.netProfit ?? 0, "profit"]].map(([label, value, kind]) => <article key={String(label)} className={`is-${kind}`}><span>{label}</span><b>{money(Number(value), "UZS")}</b></article>)}</div><div className="finance-journal"><header><div><h3>Operatsiyalar jurnali</h3><span>Onlayn: {money(summary?.onlineRevenue ?? 0, "UZS")} · Offline: {money(summary?.offlineRevenue ?? 0, "UZS")} · Hisobot xarajati: {money(summary?.offlineExpenses ?? 0, "UZS")}</span></div><Button type="button" size="sm" variant="outline" onClick={() => void load()}><RefreshCw size={15} /> Yangilash</Button></header><nav>{[["all", "Barchasi"], ["income", "Daromad"], ["expense", "Xarajat"], ["refund", "Refund"]].map(([key, label]) => <button key={key} type="button" className={filter === key ? "is-active" : ""} onClick={() => setFilter(key as typeof filter)}>{label}</button>)}</nav><div className="inventory-table-wrap"><table className="inventory-table finance-table finance-table--compact"><thead><tr><th>Chek / operatsiya</th><th>Tur</th><th>To‘lov / kategoriya</th><th>Sana</th><th>Izoh</th><th>Summa</th></tr></thead><tbody>{visible.map((item) => { const openable = Boolean(item.order); const kind = item.source === "order" ? "Buyurtma" : item.source === "refund" ? "Refund" : item.source === "offline_sale" ? "Kassa" : item.source === "offline_report" ? "Hisobot" : "Qo‘lda"; return <tr key={item.id} className={openable ? "is-openable" : ""} onClick={() => openable && setSelected(item)}><td><button type="button" className="finance-transaction-link" disabled={!openable} onClick={() => openable && setSelected(item)}><b>{item.title}</b><small>{openable ? "Chek tafsilotlarini ko‘rish" : item.source === "offline_sale" ? "Offline kassadan avtomatik" : item.source === "offline_report" ? "Offline hisobotdan" : "Qo‘lda kiritilgan"}</small></button></td><td><span className={`finance-kind is-${item.type}`}>{kind}</span></td><td><b>{item.category}</b>{item.order?.customerName && <small>{item.order.customerName}{item.order.phone ? ` · ${item.order.phone}` : ""}</small>}</td><td><time>{readableDate(item.occurredAt)}</time></td><td><span className="finance-note">{item.note?.trim() || "Izoh yozilmagan"}</span></td><td><strong className={item.amount < 0 ? "is-expense" : "is-income"}>{item.amount < 0 ? "−" : "+"}{money(Math.abs(item.amount), item.currencyCode)}</strong></td></tr>; })}{!visible.length && <tr><td colSpan={6}><Empty>Tanlangan turda operatsiya yo‘q.</Empty></td></tr>}</tbody></table></div></div>{selected && <div className="inventory-confirm-backdrop" onMouseDown={() => setSelected(null)}><section className="inventory-confirm finance-transaction-dialog" onMouseDown={(event) => event.stopPropagation()}><button className="reference-modal-close" onClick={() => setSelected(null)}>×</button><p className="ui-overline">{selected.source === "refund" ? "REFUND CHEKI" : "BUYURTMA CHEKI"}</p><h3>{selected.title}</h3><div className="finance-transaction-amount"><span>{selected.amount < 0 ? "Qaytarilgan summa" : "To‘langan summa"}</span><b className={selected.amount < 0 ? "is-expense" : "is-income"}>{selected.amount < 0 ? "−" : "+"}{money(Math.abs(selected.amount), selected.currencyCode)}</b></div>{selected.order && <><dl><div><dt>Mijoz</dt><dd>{selected.order.customerName}</dd></div><div><dt>Telefon</dt><dd>{selected.order.phone || "—"}</dd></div><div><dt>To‘lov</dt><dd>{selected.order.paymentMethod || "—"} · {selected.order.paymentStatus || "—"}</dd></div><div><dt>Yetkazish holati</dt><dd>{selected.order.fulfillmentStatus || "—"}</dd></div></dl><div className="finance-transaction-items">{selected.order.items.map((item, index) => <p key={`${item.sku}-${index}`}><span><b>{item.title}</b><small>{item.sku || "SKU yo‘q"} × {item.quantity}</small></span><strong>{money(item.total, selected.currencyCode)}</strong></p>)}</div></>}{selected.note && <p className="finance-transaction-note"><b>Izoh:</b> {selected.note}</p>}<Button type="button" variant="outline" onClick={() => setSelected(null)}>Yopish</Button></section></div>}{showForm && <div className="inventory-confirm-backdrop" onMouseDown={() => setShowForm(false)}><form className="inventory-confirm team-form finance-entry-form" onSubmit={save} onMouseDown={(event) => event.stopPropagation()}><p className="ui-overline">YANGI OPERATSIYA</p><h3>Daromad yoki xarajat kiriting</h3><div className="ui-form ui-form-grid"><Field label="Turi"><select value={draft.type} onChange={(event) => setDraft({ ...draft, type: event.target.value })}><option value="expense">Xarajat</option><option value="income">Daromad</option></select></Field><Field label="Sana"><input type="date" value={draft.occurredAt} onChange={(event) => setDraft({ ...draft, occurredAt: event.target.value })} /></Field><Field label="Nomi"><input required value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} placeholder="Masalan: kontent suratga olish" /></Field><Field label="Kategoriya"><input required value={draft.category} onChange={(event) => setDraft({ ...draft, category: event.target.value })} placeholder="Marketing, logistika…" /></Field><Field label="Summa, UZS"><input required min="1" type="number" value={draft.amount} onChange={(event) => setDraft({ ...draft, amount: event.target.value })} /></Field></div><Field label="Izoh"><textarea value={draft.note} onChange={(event) => setDraft({ ...draft, note: event.target.value })} /></Field><div className="inventory-confirm-actions"><Button type="button" variant="outline" onClick={() => setShowForm(false)}>Bekor qilish</Button><Button disabled={loading}>Saqlash</Button></div></form></div>}</section>;
}
...
type OrderWorkflowAction = { label: string; icon: "accept" | "accept-pickup" | "pack" | "ship" | "deliver" | "ready" | "pickup"; payload: { status: string; fulfillmentStatus: string } };
function nextOrderWorkflowAction(order: Order): OrderWorkflowAction | null {
  ...
  if (!pickup) return { label: "Jo‘natma blankasini to‘ldirish", icon: "accept", payload: { status: order.status, fulfillmentStatus: order.fulfillmentStatus || "pending_acceptance" } };
  if (["pending_acceptance", "unfulfilled", "awaiting_pickup", ""].includes(status)) return { label: "Qabul qilish", icon: pickup ? "accept-pickup" : "accept", payload: { status: "pending", fulfillmentStatus: pickup ? "accepted_pickup" : "accepted" } };
  if (pickup && status === "accepted_pickup") return { label: "Olib ketishga tayyor", icon: "ready", payload: { status: "pending", fulfillmentStatus: "ready_for_pickup" } };
  if (pickup && status === "ready_for_pickup") return { label: "Olib ketildi", icon: "pickup", payload: { status: "delivered", fulfillmentStatus: "picked_up" } };
  if (!pickup && status === "accepted") return { label: "Yig‘ildi", icon: "pack", payload: { status: "processing", fulfillmentStatus: "processing" } };
  if (!pickup && status === "processing") return { label: "Yo‘lga jo‘natish", icon: "ship", payload: { status: "shipped", fulfillmentStatus: "shipped" } };
  if (!pickup && status === "shipped") return { label: "Yetkazildi", icon: "deliver", payload: { status: "delivered", fulfillmentStatus: "delivered" } };
  return null;
}
function OrderActionIcon({ action }: { action: OrderWorkflowAction | null }) {
  if (!action) return <CheckCheck size={16} />;
  const Icon = action.icon === "accept" ? Truck : action.icon === "accept-pickup" ? MapPin : action.icon === "pack" ? Package : action.icon === "ship" ? ArrowUp : action.icon === "ready" ? Box : action.icon === "pickup" ? ShoppingBag : CheckCheck;
  return <Icon size={16} />;
}
...
type DashboardData = {
  ...
  salesProfit?: { sales?: Array<{ label: string; value: number }>; profit?: Array<{ label: string; value: number }> };
  salesAnalysis?: { revenue?: number; refunds?: number; discounts?: number; paymentMethods?: Record<string, { orders: number; revenue: number }>; revenueSeries?: Array<{ label: string; value: number }>; refundSeries?: Array<{ label: string; value: number }> };
  productAnalysis?: { products?: Array<{ label: string; value: number }>; sizes?: Array<{ label: string; value: number }>; colors?: Array<{ label: string; value: number }> };
  dropPerformance?: Array<{ id: string; title: string; launchAt: string; points: Array<{ label: string; revenue: number; orders: number; units: number }> }>;
  inventoryAnalysis?: { stock?: number; lowStock?: number; deadStock?: number; sellThrough?: number; topStock?: Array<{ label: string; value: number }>; distribution?: Array<{ label: string; value: number }> };
  customerAnalysis?: { newCustomers?: number; returningCustomers?: number; repeatPurchase?: number; newSeries?: Array<{ label: string; value: number }>; returningSeries?: Array<{ label: string; value: number }>; topCustomers?: Array<{ label: string; value: number }> };
  trafficAnalysis?: { funnel?: Array<{ label: string; value: number }>; favorites?: number; sources?: Array<{ label: string; value: number }> };
...
function UzbekistanRegionalMap({ regions, currency }: { regions: RegionalMetric[]; currency: string }) {
  ...
  const [mapView, setMapView] = useState({ x: 0, y: 0, scale: 1.18 });
  ...
  useEffect(() => { mapViewRef.current = mapView; const host = mapRef.current; if (host) host.style.transform = `translate(${mapView.x}px, ${mapView.y}px) scale(${mapView.scale})`; }, [mapView]);
  useEffect(() => { const viewport = mapRef.current?.parentElement; if (!viewport) return; let drag: { x: number; y: number; startX: number; startY: number } | null = null; const down = (event: PointerEvent) => { drag = { x: event.clientX, y: event.clientY, startX: mapViewRef.current.x, startY: mapViewRef.current.y }; viewport.setPointerCapture?.(event.pointerId); }; const move = (event: PointerEvent) => { const activeDrag = drag; if (!activeDrag) return; const nextX = activeDrag.startX + event.clientX - activeDrag.x, nextY = activeDrag.startY + event.clientY - activeDrag.y; setMapView((current) => ({ ...current, x: nextX, y: nextY })); }; const up = () => { drag = null; }; const wheel = (event: WheelEvent) => { event.preventDefault(); setMapView((current) => ({ ...current, scale: Math.max(1, Math.min(2.6, current.scale + (event.deltaY < 0 ? .12 : -.12))) })); }; viewport.addEventListener("pointerdown", down); viewport.addEventListener("pointermove", move); viewport.addEventListener("pointerup", up); viewport.addEventListener("pointercancel", up); viewport.addEventListener("wheel", wheel, { passive: false }); return () => { viewport.removeEventListener("pointerdown", down); viewport.removeEventListener("pointermove", move); viewport.removeEventListener("pointerup", up); viewport.removeEventListener("pointercancel", up); viewport.removeEventListener("wheel", wheel); }; }, []);
...
const dashboardGranularityForPeriod = (period: string) => period === "today" ? "hourly" : period === "year" ? "monthly" : "daily";
...
function SalesAnalyticsSection({ dashboard, currency }: { dashboard: DashboardData | null; currency: string }) {
  const chartCurrency = dashboard?.currency?.code ?? currency; const salesAnalysis = dashboard?.salesAnalysis; const paymentMethods = Object.entries(salesAnalysis?.paymentMethods ?? {}); const largestMethod = Math.max(...paymentMethods.map(([, values]) => values.revenue), 1);
  return <section className="analytics-sales-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / SALES</p><h2>Sales performance</h2><span>Sotuvlar, refund, chegirma va to‘lov usullari bir joyda.</span></div><aside><i /> Live data</aside></header><EcommerceMetrics metrics={[{ label: "Sales", value: dashboardMoney(salesAnalysis?.revenue ?? 0, chartCurrency), icon: BarChart3 }, { label: "Refund", value: dashboardMoney(salesAnalysis?.refunds ?? 0, chartCurrency), icon: RefreshCw }, { label: "Discount", value: dashboardMoney(salesAnalysis?.discounts ?? 0, chartCurrency), icon: Tags }, { label: "Payment methods", value: `${paymentMethods.length}`, icon: CreditCard }]} /><div className="analytics-sales-grid"><Card className="tailadmin-statistics-card analytics-sales-chart"><CardHeader className="tailadmin-statistics-head"><div><CardTitle>Sales &amp; Refund</CardTitle><CardDescription>Sales — line, refund — bar chart.</CardDescription></div><span className="analytics-chart-period">CURRENT PERIOD</span></CardHeader><CardContent>{(salesAnalysis?.revenueSeries?.length || salesAnalysis?.refundSeries?.length) ? <StatisticsChart current={salesAnalysis?.revenueSeries ?? []} previous={salesAnalysis?.refundSeries ?? []} currentLabel="Sales" previousLabel="Refund" mixed /> : <div className="dashboard-chart-wrap"><p>Tanlangan davrda ma’lumot yo‘q.</p></div>}<div className="dashboard-legend"><span><i /> Sales</span><span><i /> Refund</span></div></CardContent></Card><Card className="analytics-payment-card"><CardHeader><div><p className="ui-overline">PAYMENTS</p><CardTitle>Payment method</CardTitle><CardDescription>To‘langan buyurtmalar bo‘yicha taqsimot.</CardDescription></div></CardHeader><CardContent><div className="analytics-payment-methods">{paymentMethods.map(([method, values], index) => <article key={method}><span className={`analytics-payment-icon is-${index % 3}`}>{method.slice(0, 1).toUpperCase()}</span><div><b>{method}</b><small>{values.orders} ta buyurtma</small><em><i style={{ width: `${Math.max(6, values.revenue / largestMethod * 100)}%` }} /></em></div><strong>{dashboardMoney(values.revenue, chartCurrency)}</strong></article>)}{!paymentMethods.length && <p className="dashboard-empty">To‘lov ma’lumoti yo‘q.</p>}</div></CardContent></Card></div></section>;
}
function ProductRankingChart({ title, subtitle, items, tone, onSelect }: { title: string; subtitle: string; items: Array<{ label: string; value: number }>; tone: "blue" | "violet" | "orange"; onSelect?: (item: { label: string; value: number }) => void }) {
  const maximum = Math.max(...items.map((item) => item.value), 1);
  return <Card className={`analytics-ranking-card is-${tone}${onSelect ? " is-interactive" : ""}`}><CardHeader><div><p className="ui-overline">TOP SELLERS</p><CardTitle>{title}</CardTitle><CardDescription>{subtitle}</CardDescription></div></CardHeader><CardContent><div className="analytics-ranking-bars">{items.map((item, index) => <article key={item.label} role={onSelect ? "button" : undefined} tabIndex={onSelect ? 0 : undefined} onClick={() => onSelect?.(item)} onKeyDown={(event) => { if (onSelect && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); onSelect(item); } }}><span>{index + 1}</span><div><b>{item.label}</b><em><i style={{ width: `${Math.max(8, item.value / maximum * 100)}%` }} /></em></div><strong>{item.value}</strong></article>)}{!items.length && <p className="dashboard-empty">Tanlangan davrda savdo ma’lumoti yo‘q.</p>}</div></CardContent></Card>;
}
function ProductsAnalyticsSection({ dashboard }: { dashboard: DashboardData | null }) {
  const analysis = dashboard?.productAnalysis;
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / PRODUCTS</p><h2>Products performance</h2><span>Qaysi mahsulot, rang va size ko‘proq sotilayotganini kuzating.</span></div><aside><i /> Live data</aside></header><div className="analytics-ranking-grid"><ProductRankingChart title="Top Products" subtitle="Eng ko‘p sotilgan mahsulotlar" items={analysis?.products ?? []} tone="blue" /><ProductRankingChart title="Top Sizes" subtitle="Eng talabgir size’lar" items={analysis?.sizes ?? []} tone="violet" /><ProductRankingChart title="Top Colors" subtitle="Eng talabgir ranglar" items={analysis?.colors ?? []} tone="orange" /></div></section>;
}
function DropPerformanceSection({ dashboard, currency }: { dashboard: DashboardData | null; currency: string }) {
  const drops = dashboard?.dropPerformance ?? []; const [selectedId, setSelectedId] = useState(""); const selected = drops.find((drop) => drop.id === selectedId) ?? drops[0]; const chartCurrency = dashboard?.currency?.code ?? currency; const points = selected?.points ?? [];
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / DROPS</p><h2>Drop Performance</h2><span>Har dropning ilk 1 soat, 24 soat va 7 kundagi natijasi.</span></div><aside><i /> Live data</aside></header>{drops.length ? <><div className="analytics-drop-list">{drops.map((drop) => <button type="button" key={drop.id} className={selected?.id === drop.id ? "is-selected" : ""} onClick={() => setSelectedId(drop.id)}><b>{drop.title}</b><small>{new Intl.DateTimeFormat("uz-UZ", { day: "2-digit", month: "short", year: "numeric" }).format(new Date(drop.launchAt))}</small></button>)}</div><div className="analytics-drop-grid"><Card className="tailadmin-statistics-card"><CardHeader className="tailadmin-statistics-head"><div><CardTitle>{selected?.title}</CardTitle><CardDescription>Area — savdo, ikkinchi chiziq — buyurtmalar soni.</CardDescription></div></CardHeader><CardContent><StatisticsChart current={points.map((point) => ({ label: point.label, value: point.revenue }))} previous={points.map((point) => ({ label: point.label, value: point.orders }))} currentLabel="Revenue" previousLabel="Orders" /><div className="dashboard-legend"><span><i /> Revenue</span><span><i /> Orders</span></div></CardContent></Card><div className="analytics-drop-metrics">{points.map((point) => <article key={point.label}><span>First {point.label}</span><b>{dashboardMoney(point.revenue, chartCurrency)}</b><small>{point.orders} buyurtma · {point.units} ta mahsulot</small></article>)}</div></div></> : <Card><CardContent><Empty>Faol drop yoki unga tegishli ma’lumot hali yo‘q.</Empty></CardContent></Card>}</section>;
}
function InventoryAnalyticsSection({ dashboard }: { dashboard: DashboardData | null }) {
  const analysis = dashboard?.inventoryAnalysis;
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / INVENTORY</p><h2>Inventory health</h2><span>Ombor holati, past qoldiq va sotuv tezligini kuzating.</span></div><aside><i /> Live data</aside></header><EcommerceMetrics metrics={[{ label: "Stock", value: new Intl.NumberFormat("uz-UZ").format(analysis?.stock ?? 0), icon: Package }, { label: "Low stock", value: new Intl.NumberFormat("uz-UZ").format(analysis?.lowStock ?? 0), icon: Archive }, { label: "Dead stock", value: new Intl.NumberFormat("uz-UZ").format(analysis?.deadStock ?? 0), icon: Box }, { label: "Sell-through", value: `${(analysis?.sellThrough ?? 0).toFixed(1)}%`, icon: BarChart3 }]} /><div className="analytics-ranking-grid analytics-inventory-grid"><ProductRankingChart title="Stock by product" subtitle="Ombordagi eng katta qoldiqlar" items={analysis?.topStock ?? []} tone="blue" /><ProductRankingChart title="Stock status" subtitle="Qoldiqning holatlar bo‘yicha taqsimoti" items={analysis?.distribution ?? []} tone="orange" /></div></section>;
}
function CustomersAnalyticsSection({ dashboard, currency, customers, onOpenCustomer }: { dashboard: DashboardData | null; currency: string; customers: CustomerRecord[]; onOpenCustomer: (id: string) => void }) {
  const analysis = dashboard?.customerAnalysis; const chartCurrency = dashboard?.currency?.code ?? currency; const total = (analysis?.newCustomers ?? 0) + (analysis?.returningCustomers ?? 0); const newShare = total ? (analysis?.newCustomers ?? 0) / total * 100 : 0;
  const selectCustomer = (item: { label: string }) => { const customer = customers.find((entry) => `${entry.firstName} ${entry.lastName}`.trim() === item.label || entry.email === item.label); if (customer) onOpenCustomer(customer.id); };
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / CUSTOMERS</p><h2>Customer insights</h2><span>Yangi va qaytgan mijozlar, repeat purchase hamda eng yaxshi mijozlar.</span></div><aside><i /> Live data</aside></header><EcommerceMetrics metrics={[{ label: "New customers", value: new Intl.NumberFormat("uz-UZ").format(analysis?.newCustomers ?? 0), icon: Users }, { label: "Returning", value: new Intl.NumberFormat("uz-UZ").format(analysis?.returningCustomers ?? 0), icon: RefreshCw }, { label: "Repeat purchase", value: new Intl.NumberFormat("uz-UZ").format(analysis?.repeatPurchase ?? 0), icon: ShoppingBag }]} /><div className="analytics-customers-grid"><Card className="tailadmin-statistics-card"><CardHeader className="tailadmin-statistics-head"><div><CardTitle>New vs Returning</CardTitle><CardDescription>Vaqt bo‘yicha mijozlar dinamikasi.</CardDescription></div></CardHeader><CardContent>{(analysis?.newSeries?.length || analysis?.returningSeries?.length) ? <StatisticsChart current={analysis?.newSeries ?? []} previous={analysis?.returningSeries ?? []} currentLabel="New" previousLabel="Returning" /> : <div className="dashboard-chart-wrap"><p>Tanlangan davrda ma’lumot yo‘q.</p></div>}<div className="dashboard-legend"><span><i /> New</span><span><i /> Returning</span></div></CardContent></Card><Card className="analytics-donut-card"><CardHeader><div><p className="ui-overline">CUSTOMER MIX</p><CardTitle>New / Returning</CardTitle></div></CardHeader><CardContent><div className="analytics-donut" style={{ background: `conic-gradient(#465fff 0 ${newShare}%, #a4bcfd ${newShare}% 100%)` }}><div><b>{total}</b><span>mijoz</span></div></div><div className="analytics-donut-legend"><span><i /> New <b>{analysis?.newCustomers ?? 0}</b></span><span><i /> Returning <b>{analysis?.returningCustomers ?? 0}</b></span></div></CardContent></Card><ProductRankingChart title="Top Customers" subtitle="Davr bo‘yicha eng ko‘p xarid qilganlar" items={analysis?.topCustomers ?? []} tone="violet" onSelect={selectCustomer} /></div></section>;
}
function TrafficAnalyticsSection({ dashboard }: { dashboard: DashboardData | null }) {
  const analysis = dashboard?.trafficAnalysis; const funnel = analysis?.funnel ?? []; const max = Math.max(...funnel.map((item) => item.value), 1);
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / TRAFFIC</p><h2>Traffic &amp; Conversion</h2><span>Visitdan xaridgacha bo‘lgan mijoz yo‘lini kuzating.</span></div><aside><i /> Live data</aside></header><EcommerceMetrics metrics={[{ label: "Visits", value: new Intl.NumberFormat("uz-UZ").format(funnel[0]?.value ?? 0), icon: Users }, { label: "Add to cart", value: new Intl.NumberFormat("uz-UZ").format(funnel[2]?.value ?? 0), icon: ShoppingBag }, { label: "Purchase", value: new Intl.NumberFormat("uz-UZ").format(funnel[4]?.value ?? 0), icon: CreditCard }, { label: "Favorites", value: new Intl.NumberFormat("uz-UZ").format(analysis?.favorites ?? 0), icon: Tags }]} /><div className="analytics-traffic-grid"><Card className="analytics-funnel-card"><CardHeader><div><p className="ui-overline">CONVERSION FUNNEL</p><CardTitle>Customer journey</CardTitle><CardDescription>Har bosqichdagi foydalanuvchilar soni.</CardDescription></div></CardHeader><CardContent><div className="analytics-funnel">{funnel.map((item, index) => <article key={item.label} style={{ width: `${Math.max(34, item.value / max * 100)}%` }}><span>{index + 1}</span><b>{item.label}</b><strong>{item.value.toLocaleString("uz-UZ")}</strong></article>)}{!funnel.length && <p className="dashboard-empty">Traffic ma’lumoti yo‘q.</p>}</div></CardContent></Card><ProductRankingChart title="Traffic sources" subtitle="Kanal va conversion bosqichlari bo‘yicha oqim" items={analysis?.sources ?? []} tone="orange" /></div></section>;
}
function MarketingAnalyticsSection({ dashboard, currency }: { dashboard: DashboardData | null; currency: string }) {
  const chartCurrency = dashboard?.currency?.code ?? currency; const sales = dashboard?.salesAnalysis; const totalRevenue = sales?.revenue ?? 0; const promoRevenue = sales?.discounts ?? 0; const sources = [{ label: "Direct", revenue: Math.max(0, totalRevenue - promoRevenue), orders: dashboard?.kpis?.current?.orders ?? 0 }, { label: "Promo codes", revenue: promoRevenue, orders: 0 }, { label: "Instagram", revenue: 0, orders: 0 }, { label: "Google", revenue: 0, orders: 0 }, { label: "Campaigns", revenue: 0, orders: 0 }];
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / MARKETING</p><h2>Marketing performance</h2><span>Kanallar, promokodlar va kampaniyalarning savdoga ta’siri.</span></div><aside><i /> Live data</aside></header><EcommerceMetrics metrics={[{ label: "Revenue", value: dashboardMoney(totalRevenue, chartCurrency), icon: BarChart3 }, { label: "Orders", value: new Intl.NumberFormat("uz-UZ").format(dashboard?.kpis?.current?.orders ?? 0), icon: ShoppingBag }, { label: "Promo discount", value: dashboardMoney(promoRevenue, chartCurrency), icon: Tags }, { label: "Active sources", value: `${sources.filter((source) => source.revenue > 0 || source.orders > 0).length}`, icon: Monitor }]} /><div className="analytics-marketing-grid"><Card className="tailadmin-statistics-card"><CardHeader className="tailadmin-statistics-head"><div><CardTitle>Revenue &amp; Orders by source</CardTitle><CardDescription>Line — revenue, bar — orders.</CardDescription></div></CardHeader><CardContent><StatisticsChart current={sources.map((source) => ({ label: source.label, value: source.revenue }))} previous={sources.map((source) => ({ label: source.label, value: source.orders }))} currentLabel="Revenue" previousLabel="Orders" mixed /><div className="dashboard-legend"><span><i /> Revenue</span><span><i /> Orders</span></div></CardContent></Card><ProductRankingChart title="Channel revenue" subtitle="Manbalar bo‘yicha natija" items={sources.map((source) => ({ label: source.label, value: source.revenue }))} tone="orange" /></div></section>;
}
function GeographyAnalyticsSection({ customers, orders, currency }: { customers: CustomerRecord[]; orders: Order[]; currency: string }) {
  const regions = regionalMetrics(customers, orders).sort((left, right) => right.revenue - left.revenue);
  return <section className="analytics-products-section"><header className="analytics-sales-heading"><div><p className="ui-overline">ANALYTICS / GEOGRAPHY</p><h2>Geography</h2><span>Toshkent va viloyatlar bo‘yicha savdo hamda mijozlar taqsimoti.</span></div><aside><i /> Uzbekistan</aside></header><div className="analytics-geography-grid"><UzbekistanRegionalMap regions={regions} currency={currency} /><ProductRankingChart title="Top Regions" subtitle="To‘langan savdo bo‘yicha hududlar" items={regions.filter((region) => region.revenue > 0).map((region) => ({ label: region.label, value: region.revenue }))} tone="blue" /></div></section>;
}
function DashboardOverview({ dashboard, currency, period, metric, granularity, customFrom, customTo, customers, orders, onCurrency, onPeriod, onMetric, onGranularity, onCustomFrom, onCustomTo, heading = "Overview Analytics" }: { dashboard: DashboardData | null; currency: string; period: string; metric: string; granularity: string; customFrom: string; customTo: string; customers: CustomerRecord[]; orders: Order[]; onCurrency: (value: string) => void; onPeriod: (value: string) => void; onMetric: (value: string) => void; onGranularity: (value: string) => void; onCustomFrom: (value: string) => void; onCustomTo: (value: string) => void; heading?: string }) {
  const kpis = dashboard?.kpis?.current ?? {}; const changes = dashboard?.kpis?.changes ?? {}; const chart = dashboard?.chart; const chartCurrency = dashboard?.currency?.code ?? currency; const sales = dashboard?.salesProfit?.sales ?? []; const profit = dashboard?.salesProfit?.profit ?? [];
  const cards: Array<[string, string, typeof Package, boolean]> = [["Revenue", "revenue", BarChart3, true], ["Net Profit", "profit", ArrowUp, true], ["Orders", "orders", ClipboardList, false], ["AOV", "averageOrderValue", CreditCard, true], ["Conversion", "conversion", SlidersHorizontal, false], ["Visitors", "visitors", Users, false]];
  ...
  const metricCards = cards.map(([label, key, Icon, financial]) => { const count = new Intl.NumberFormat("uz-UZ").format(kpis[key] ?? 0); return { label, icon: Icon, change: changes[key], value: financial ? dashboardMoney(kpis[key] ?? 0, chartCurrency) : key === "conversion" ? `${(kpis[key] ?? 0).toFixed(1)}%` : count }; });
  return <section className="dashboard-overview">
    <header className="dashboard-controls"><div><p className="ui-overline">{heading === "Dashboard" ? "DASHBOARD" : "ANALYTICS"}</p><h2>{heading}</h2></div><div className="dashboard-control-fields"><label>Valyuta<select value={currency} onChange={(event) => onCurrency(event.target.value)}>{["UZS", "USD", "EUR", "RUB", "KZT"].map((code) => <option key={code} value={code}>{code}</option>)}</select></label></div></header>
    ...
    <Card className="dashboard-chart-card tailadmin-statistics-card"><CardHeader className="tailadmin-statistics-head"><div><CardTitle>Sales &amp; Net Profit</CardTitle><CardDescription>Vaqt bo‘yicha savdo va sof foyda dinamikasi.</CardDescription></div><div className="tailadmin-statistics-controls"><label className="tailadmin-statistics-period"><CalendarDays size={18} /><select value={period} onChange={(event) => { const value = event.target.value; onPeriod(value); onGranularity(dashboardGranularityForPeriod(value)); }}>{DASHBOARD_PERIODS.map(([value, label]) => <option key={value} value={value}>{label}</option>)}</select></label></div></CardHeader><CardContent>{sales.length || profit.length ? <StatisticsChart current={sales} previous={profit} currentLabel="Sales" previousLabel="Net Profit" /> : <div className="dashboard-chart-wrap"><p>Tanlangan davrda haqiqiy ma’lumot yo‘q.</p></div>}<div className="dashboard-legend"><span><i /> Sales</span><span><i /> Net Profit</span></div></CardContent></Card>
    ...
    {heading === "Dashboard" && <><UzbekistanRegionalMap regions={regionalMetrics(customers, orders)} currency={chartCurrency} />
...
export default function AdminConsole() {
  ...
  const [shipmentDraft, setShipmentDraft] = useState({ provider: "UzPost", senderName: "SIREN", senderPhone: "", weightKg: "1", packageCount: "1", comment: "" });
  const shipmentDraftRef = useRef(shipmentDraft);
  ...
  const refresh = useCallback(
    async (t = tab) => {
      if (t === "dashboard" || t === "analytics" || t === "analytics_sales" || t === "analytics_products" || t === "analytics_drops" || t === "analytics_inventory" || t === "analytics_customers" || t === "analytics_traffic" || t === "analytics_marketing" || t === "analytics_geography") {
      ...
      if (t === "products" || t === "catalog" || t === "discounts" || t === "partners" || t === "business_cost" || t === "history_cost") await catalog();
  ...
  const advanceOrder = async (order: Order) => {
    if (order.shippingAddress?.fulfillmentMethod !== "pickup") { openShipmentForm(order); return; }
  ...
  const openShipmentForm = (order: Order) => { setShipmentDraft((current) => ({ ...current, senderPhone: current.senderPhone || order.customer?.phone || "" })); setDeliveryOrder(order); };
  const submitShipment = async (event: FormEvent) => {
    event.preventDefault(); if (!deliveryOrder) return;
    setShipmentSaving(true);
    try {
      await api(`/admin/orders/${deliveryOrder.id}/shipment`, token, { method: "POST", body: JSON.stringify(shipmentDraftRef.current) });
      setNotice(`Chek #${deliveryOrder.orderNumber} uchun jo‘natma blankasi saqlandi. Keyingi holatlar hamkor API orqali keladi.`);
      setDeliveryOrder(null); await refresh("orders");
    } catch (reason) { setNotice(reason instanceof Error ? reason.message : "Jo‘natma blankasini saqlab bo‘lmadi."); } finally { setShipmentSaving(false); }
  };
  useEffect(() => { shipmentDraftRef.current = shipmentDraft; }, [shipmentDraft]);
  useEffect(() => {
    if (!deliveryOrder || deliveryOrder.shippingAddress?.fulfillmentMethod === "pickup") return;
    const modal = document.querySelector<HTMLElement>(".reference-delivery-modal"); if (!modal || modal.querySelector("[data-shipment-form]")) return;
    const action = modal.querySelector<HTMLElement>(".ui-button"); if (action) action.style.display = "none";
    const slot = document.createElement("div"); slot.dataset.shipmentForm = "true"; modal.appendChild(slot); const root = createRoot(slot);
    root.render(<form className="ui-form reference-shipment-form" onSubmit={submitShipment}><p><b>Hamkor:</b> jo‘natma yaratilgandan keyin statuslar faqat hamkor API/webhookidan avtomatik yangilanadi.</p><div className="ui-form-grid"><Field label="Yetkazib berish hamkori"><select defaultValue={shipmentDraft.provider} onChange={(event) => setShipmentDraft((current) => ({ ...current, provider: event.target.value }))}><option>UzPost</option><option>EMU</option><option>Express24</option><option>Boshqa</option></select></Field><Field label="Jo‘natuvchi"><input required defaultValue={shipmentDraft.senderName} onChange={(event) => setShipmentDraft((current) => ({ ...current, senderName: event.target.value }))} /></Field><Field label="Jo‘natuvchi telefoni"><input required defaultValue={shipmentDraft.senderPhone} onChange={(event) => setShipmentDraft((current) => ({ ...current, senderPhone: event.target.value }))} /></Field><Field label="Og‘irligi, kg"><input required inputMode="decimal" defaultValue={shipmentDraft.weightKg} onChange={(event) => setShipmentDraft((current) => ({ ...current, weightKg: event.target.value }))} /></Field><Field label="Joylar soni"><input required inputMode="numeric" defaultValue={shipmentDraft.packageCount} onChange={(event) => setShipmentDraft((current) => ({ ...current, packageCount: event.target.value }))} /></Field></div><Field label="Izoh"><textarea defaultValue={shipmentDraft.comment} onChange={(event) => setShipmentDraft((current) => ({ ...current, comment: event.target.value }))} /></Field><Button disabled={shipmentSaving}>{shipmentSaving ? "Saqlanmoqda…" : "Hamkor uchun jo‘natmani tayyorlash"}</Button></form>);
    return () => { window.setTimeout(() => { root.unmount(); slot.remove(); }, 0); };
  }, [deliveryOrder, shipmentSaving]);
  useEffect(() => {
  ...
  const nav: Array<[Tab, string, ElementType]> = [
    ["dashboard", "Dashboard", TailAdminGridIcon],
    ["analytics", "Overview Analytics", BarChart3],
    ["analytics_sales", "Sales", BarChart3],
    ["analytics_products", "Products", Package],
    ["analytics_drops", "Drop Performance", CalendarDays],
    ["analytics_inventory", "Inventory", Archive],
    ["analytics_customers", "Customers", Users],
    ["analytics_traffic", "Traffic & Conversion", BarChart3],
    ["analytics_marketing", "Marketing", Monitor],
    ["analytics_geography", "Geography", MapPin],
    ...
    ["report_online", "Online hisobot", BarChart3],
    ["report_offline", "Offline hisobot", BarChart3],
    ["report_combined", "Umumiy hisobot", BarChart3],
    ["business_cost", "Biznesning asl narxi", Archive],
    ["history_cost", "Asl narx tarixi", Archive],
    ["history_sales", "Sotuv tarixi", BarChart3],
    ["history_units", "Sotilgan birliklar", ShoppingBag],
    ["payments", "Payme", CreditCard],
  ...
  const standardNavGroups: Array<[string, Array<[string, string, ElementType]>]> = [
    ["Asosiy", [nav[0], ...nav.slice(10, 14)]],
    ["Analytics", [nav[1], nav[2], nav[3], nav[4], nav[5], nav[6], nav[7], nav[8], nav[9]]],
    ["Savdo", nav.filter(([id]) => ["pickup", "customers", "discounts", "partners", "finance", "report_online", "report_offline", "report_combined", "business_cost", "payments", "currencies"].includes(id))],
    ["Offline do‘kon", nav.filter(([id]) => id.startsWith("offline_"))],
    ["Tarix", nav.filter(([id]) => id === "history_cost" || id === "history_sales" || id === "history_units")],
    ["Kontent", [...contentSidebarNav, ...nav.filter(([id]) => id === "pages" || id === "notifications")]],
    ["Tizim", nav.filter(([id]) => id === "team" || id === "audit")],
  ];
  const offlineNav = nav.filter(([id]) => id.startsWith("offline_"));
  ...
  return (
    ...
    >
      <AdminShell activeId={activeSidebarId} groups={navGroups.map(([label, items]) => ({ label, items: items.map(([id, itemLabel, icon]) => ({ id, label: itemLabel, icon })) }))} onNavigate={(id) => { if (id.startsWith("content-")) { setContentSubsection(id.slice("content-".length) as ContentSubsection); setTab("content"); void run(() => refresh("content")); return; } setTab(id as Tab); void run(() => refresh(id as Tab)); }} compact={sidebarCompact} onCompactChange={setSidebarCompact} title={tab === "content" ? activeContentLabel : nav.find((item) => item[0] === tab)?.[1] ?? "Admin"} theme={adminTheme} onThemeChange={(value) => setAdminTheme(value as "light" | "midnight" | "violet" | "graphite" | "forest")} profile={adminProfile} busy={loading} onRefresh={() => void run(async () => { await refresh(); await refresh("dashboard"); })} notificationCount={unreadActivities.length} notificationContent={<section className="admin-activity-notifications"><header><div><p>YANGI FAOLLIK</p><b>Bildirishnomalar</b></div>{unreadActivities.length > 0 && <button type="button" onClick={() => markActivitiesRead(unreadActivities.map((item) => item.id))}><CheckCheck size={15} /> Barchasini o‘qish</button>}</header><div className="admin-notification-tabs"><button type="button" className={notificationSection === "system" ? "is-active" : ""} onClick={() => setNotificationSection("system")}>Tizim <i>{systemActivities.filter((item) => !readActivityIds.includes(item.id)).length}</i></button><button type="button" className={notificationSection === "orders" ? "is-active" : ""} onClick={() => setNotificationSection("orders")}>Buyurtmalar <i>{orderActivities.filter((item) => !readActivityIds.includes(item.id)).length}</i></button></div>{notificationSection === "orders" && <div className="admin-order-notification-permission"><span>{browserNotificationPermission === "granted" ? "Chrome bildirishnomasi yoqilgan" : "Ovozli Chrome bildirishnomalarini yoqing"}</span>{browserNotificationPermission !== "granted" && <button type="button" onClick={() => void enableOrderNotifications()}>Yoqish</button>}</div>}{visibleActivities.length ? <div className="admin-activity-notification-list">{visibleActivities.slice(0, 12).map((item) => { const isRead = readActivityIds.includes(item.id); const isOrder = item.entityType === "order"; return <button type="button" key={item.id} className={`admin-activity-notification${isRead ? " is-read" : ""}${isOrder ? " is-order" : ""}`} onClick={() => { markActivitiesRead([item.id]); if (isOrder) void openOrderNotification(item.entityId); }}><i aria-hidden="true" /><span><b>{dashboardActivityTitle(item.action, item.entityType)}</b><small>{item.user} · {dashboardTime(item.createdAt)}{isOrder ? " · Chekni ochish" : ""}</small></span></button>; })}</div> : <p className="admin-activity-empty">Bu bo‘limda bildirishnoma yo‘q.</p>}</section>} onSignOut={() => { signOutAdmin(); setToken(""); }}>
        ...
        <TabsContent value="analytics_sales"><SalesAnalyticsSection dashboard={dashboard as DashboardData | null} currency={dashboardCurrency} /></TabsContent>
        <TabsContent value="analytics_products"><ProductsAnalyticsSection dashboard={dashboard as DashboardData | null} /></TabsContent>
        <TabsContent value="analytics_drops"><DropPerformanceSection dashboard={dashboard as DashboardData | null} currency={dashboardCurrency} /></TabsContent>
        <TabsContent value="analytics_inventory"><InventoryAnalyticsSection dashboard={dashboard as DashboardData | null} /></TabsContent>
        <TabsContent value="analytics_customers"><CustomersAnalyticsSection dashboard={dashboard as DashboardData | null} currency={dashboardCurrency} customers={customers} onOpenCustomer={(id) => void openOrderCustomer(id)} /></TabsContent>
        <TabsContent value="analytics_traffic"><TrafficAnalyticsSection dashboard={dashboard as DashboardData | null} /></TabsContent>
        <TabsContent value="analytics_marketing"><MarketingAnalyticsSection dashboard={dashboard as DashboardData | null} currency={dashboardCurrency} /></TabsContent>
        <TabsContent value="analytics_geography"><GeographyAnalyticsSection customers={customers} orders={orders} currency={dashboardCurrency} /></TabsContent>
        ...
        <TabsContent value="report_online"><BusinessReport scope="online" token={token} /></TabsContent>
        <TabsContent value="report_offline"><BusinessReport scope="offline" token={token} /></TabsContent>
        <TabsContent value="report_combined"><BusinessReport scope="combined" token={token} /></TabsContent>
        <TabsContent value="business_cost"><BusinessCostOverview products={products} /></TabsContent>
        <TabsContent value="history_cost"><CostHistoryChart products={products} /></TabsContent>
        <TabsContent value="history_sales"><SalesHistoryChart token={token} /></TabsContent>
        <TabsContent value="history_units"><UnitsHistoryChart token={token} /></TabsContent>
        ...
        <TabsContent value="payments"><section className="tailadmin-products-page payme-admin-page"><header className="tailadmin-page-heading reference-products-heading"><div><p className="ui-overline">TO‘LOVLAR</p><h2>Payme</h2><span>Payme Business bilan onlayn to‘lov, callback va tranzaksiyalar holatini boshqaring.</span></div><nav aria-label="Breadcrumb" className="reference-breadcrumb"><span>Bosh sahifa</span><ChevronRight size={15} /><b>Payme</b></nav></header><PaymeIntegrationPanel token={token} onNotify={setNotice} /></section></TabsContent>
