...
function CategoryManager({ categories, products, token, onChanged }: { categories: Taxonomy[]; products: Product[]; token: string; onChanged: () => Promise<void> }) {
  ...
  return <Card className="category-manager category-manager--editor">
    ...
    <CardContent><section className="category-editor">
      ...
      {editing.id ? <div className="category-product-list" aria-label="Kategoriya mahsulotlari">{visible.map((product) => { const assigned = product.categoryId === editing.id; const owner = categories.find((category) => category.id === product.categoryId); return <button type="button" key={product.id} className={assigned ? "is-assigned" : ""} disabled={busy} onClick={() => !assigned && void move(product)}><span>{assigned ? "✓" : ""}</span><div><b>{product.title}</b><small>{assigned ? "Shu kategoriya" : owner ? `Hozir: ${owner.name}` : "Kategoriya biriktirilmagan"}</small></div><em>{assigned ? "Biriktirilgan" : "Biriktirish"}</em></button>; })}{!visible.length && <Empty>Mahsulot topilmadi.</Empty>}</div> : <p className="category-save-note">Mahsulotlarni tanlash uchun avval kategoriyani saqlang.</p>}
