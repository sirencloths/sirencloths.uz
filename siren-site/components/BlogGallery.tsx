...
export default function BlogGallery({ postId, title, cover, images }: Props) {
  ...
  return <div className="blog-page-gallery">
    ...
    {count > 0 && <section className="blog-gallery-slider" aria-label={`${title} galereyasi`}>
      <div className="blog-gallery-slider__items">{visibleImages.map(({ image, index }) => <Link className="blog-gallery-slider__image" key={`${image}-${index}`} href={`/blog/${postId}/${index}`} aria-label={`${index + 1}-rasmni alohida sahifada ochish`}><img src={image} alt={`${title} — ${index + 1}`} /></Link>)}</div>
