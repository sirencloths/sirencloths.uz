      ...
      <header className="sticky top-0 z-30 flex w-full border-b border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900" style={{ ...surfaceStyle, borderColor: "#e4e7ec" }}>
        <div className="flex w-full grow flex-col items-center justify-between xl:flex-row xl:px-6">
          <div className="flex w-full items-center gap-3 border-b border-gray-200 px-3 py-3 xl:border-b-0 xl:px-0 xl:py-4 dark:border-gray-800">
            <button type="button" aria-label="Menyuni ochish" onClick={toggleSidebar} className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-100 lg:h-11 lg:w-11 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-white/5" style={{ borderColor: "#d0d5dd" }}>{mobileOpen ? <X size={20} /> : <Menu size={19} />}</button>
            <div className="hidden xl:block"><p className="text-xs font-medium text-gray-400">Control Center</p><h1 className="text-lg font-semibold text-gray-800 dark:text-white/90">{title}</h1></div>
            <div className="relative ml-auto hidden xl:block xl:ml-8"><Search size={19} className="pointer-events-none absolute top-1/2 left-4 -translate-y-1/2 text-gray-500" /><input ref={searchRef} type="search" placeholder="Qidirish..." className="h-11 w-107.5 rounded-lg border border-gray-200 bg-transparent py-2.5 pr-14 pl-12 text-sm text-gray-800 shadow-theme-xs outline-hidden placeholder:text-gray-400 focus:border-brand-300 focus:ring-3 focus:ring-brand-500/10 dark:border-gray-800 dark:bg-white/3 dark:text-white/90" style={{ borderColor: "#d0d5dd" }} /><span className="absolute top-1/2 right-2.5 -translate-y-1/2 rounded-lg border border-gray-200 bg-gray-50 px-1.5 py-1 text-xs text-gray-500 dark:border-gray-800 dark:bg-white/3" style={{ borderColor: "#d0d5dd" }}>⌘ K</span></div>
          ...
          <div className="flex w-full items-center justify-end gap-3 px-4 py-3 xl:w-auto xl:px-0 xl:py-4">
            <label className="flex h-10 items-center gap-2 rounded-lg border border-gray-200 px-3 text-sm text-gray-600 dark:border-gray-800 dark:text-gray-300" style={{ borderColor: "#d0d5dd" }}><span>{theme === "light" ? <Sun size={17} /> : <Moon size={17} />}</span><select value={theme} onChange={(event) => onThemeChange(event.target.value)} className="bg-transparent text-sm outline-hidden"><option value="light">Light</option><option value="midnight">Dark</option><option value="graphite">Contrast</option><option value="forest">Brand</option></select></label>
            <div className="relative"><button type="button" aria-label="Bildirishnomalar" onClick={() => setNotificationsOpen((open) => !open)} className="dropdown-toggle relative flex h-10 w-10 items-center justify-center rounded-lg border border-gray-200 text-gray-500 hover:bg-gray-100 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-white/5" style={{ borderColor: "#d0d5dd" }}><Bell size={19} />{notificationCount > 0 && <span className="absolute -top-1 -right-1 min-w-4 rounded-full bg-brand-500 px-1 text-center text-[10px] font-bold leading-4 text-white">{notificationCount > 9 ? "9+" : notificationCount}</span>}</button><Dropdown isOpen={notificationsOpen} onClose={() => setNotificationsOpen(false)} className="w-80 overflow-hidden">{notificationContent}</Dropdown></div>
