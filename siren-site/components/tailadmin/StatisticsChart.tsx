...
export default function StatisticsChart({ current, previous }: { current: ChartPoint[]; previous: ChartPoint[] }) {
  ...
  return <div className="tailadmin-statistics-chart"><Chart options={options} series={[{ name: "Joriy davr", data: labels.map((label) => valueFor(current, label)) }, { name: "Oldingi davr", data: labels.map((label) => valueFor(previous, label)) }]} type="area" height={310} /></div>;
}
