...
type Partner = { id: string; name: string; logoUrl?: string | null; showInCheckoutSummary?: boolean; type: string; email?: string; phone?: string; promoCode?: string; discountPercent: number; productIds: string[]; isActive: boolean; usageCount: number; commentCount: number; latestComment?: string | null; createdAt: string };
...
export default function PartnerManager({ token, products, onNotice }: { token: string; products: Product[]; onNotice: (message: string) => void }) {
  ...
  const [draft, setDraft] = useState({ name: "", logoUrl: "", showInCheckoutSummary: false, type: "sponsor", email: "", phone: "", promoCode: "", discountPercent: 10, note: "", productIds: [] as string[] });
  ...
  return <section className="tailadmin-partners-page">
    ...
    <Modal isOpen={open} onClose={() => setOpen(false)} className="max-w-3xl !bg-white dark:!bg-white" showCloseButton>
      ...
      <label>Logo manzili<input type="url" value={draft.logoUrl} onChange={(event) => setDraft({ ...draft, logoUrl: event.target.value })} placeholder="https://.../payme-logo.svg" /></label>
      <label className="partner-summary-toggle"><input type="checkbox" checked={draft.showInCheckoutSummary} onChange={(event) => setDraft({ ...draft, showInCheckoutSummary: event.target.checked })} /> Checkout summary’da ko‘rsatish</label>
