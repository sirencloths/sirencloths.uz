...
export class AdminService {
  ...
  async dashboard(input: DashboardInput = {}) {
    ...
    const monthlySales = Array.from({ length: 12 }, (_, month) => { const value = orders.filter((order) => paidOrder(order) && order.createdAt.getFullYear() === range.end.getFullYear() && order.createdAt.getMonth() === month).reduce((sum, order) => sum + numberOf(order.totalAmount), 0); return { label: new Intl.DateTimeFormat('en-US', { month: 'short' }).format(new Date(2020, month, 1)), value: currencyReady ? this.convert(value, currency, rates) : value }; });
