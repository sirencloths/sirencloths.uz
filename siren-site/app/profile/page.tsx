...
export default function ProfilePage() {
  ...
  return <><FixedTop /><main className="profile-page">
    {loading ? <section className="profile-box"><p className="profile-empty">ЗAGRUZKA...</p></section> : !customer ? <section className="profile-box profile-empty"><p>ВОЙДИТЕ, ЧТОБЫ УВИДЕТЬ ПРОФИЛЬ И ЗАКАЗЫ</p><button type="button" onClick={openAuth}>SIGN IN / REGISTER</button></section> : <>
      ...
      {customer.welcomeDiscountEligible && <section className="profile-box"><b>WELCOME OFFER</b><p>Ваша скидка {customer.welcomeDiscountPercent || 15}% будет применена к первой покупке.</p>{customer.welcomeDiscountExpiresAt && <small>24 soatlik aksiya: {new Intl.DateTimeFormat("uz-UZ", { dateStyle: "medium", timeStyle: "short" }).format(new Date(customer.welcomeDiscountExpiresAt))} gacha.</small>}</section>}
