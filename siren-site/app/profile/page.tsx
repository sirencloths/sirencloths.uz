...
export default function ProfilePage() {
  ...
  const activeOrders = orders.filter((order) => { const value = stage(order); return value >= 0 && value < 3; });
  ...
  return <><main className="profile-page">
    {loading ? <section className="profile-box"><p className="profile-empty">ЗAGRUZKA...</p></section> : !customer ? <section className="profile-box profile-empty"><p>ВОЙДИТЕ, ЧТОБЫ УВИДЕТЬ ПРОФИЛЬ И ЗАКАЗЫ</p><button type="button" onClick={openAuth}>SIGN IN / REGISTER</button></section> : <>
      ...
      <section className="customer-orders"><header><div><p>BUYURTMALAR</p><h2>Faol buyurtmalar</h2></div><div className="customer-orders-head-actions"><span>{activeOrders.length} ta</span><Link href="/profile/orders">Barcha buyurtmalar →</Link></div></header>{ordersLoading ? <p className="profile-empty">Buyurtmalar yuklanmoqda…</p> : activeOrders.map((order) => { const active = stage(order); const labels = ["Qabul qilindi", "Yig‘ilyapti", "Yo‘lda", "Yetkazildi"]; return <article className={`customer-order ${active < 0 ? "is-cancelled" : ""}`} key={order.id}><header><div><b>Chek #{order.orderNumber}</b><small>{new Intl.DateTimeFormat("uz-UZ", { dateStyle: "medium", timeStyle: "short" }).format(new Date(order.createdAt))}</small></div><span className={`customer-order-status stage-${Math.max(0, active)}`}>{active < 0 ? "Bekor qilingan" : labels[active]}</span></header><div className="customer-order-items">{order.items.map((item) => <div key={item.id}><img src={image(item.imageUrl)} alt="" /><span><b>{item.titleSnapshot}</b><small>{item.skuSnapshot || "SKU"} × {item.quantity}</small></span><strong>{money(item.totalPrice)}</strong></div>)}</div><div className="customer-order-tracking"><div className="customer-tracking-code">TRACKING: SIR-{String(order.orderNumber).padStart(6, "0")}</div><ol>{labels.map((label, index) => <li className={index <= active ? "is-done" : ""} key={label}><i>{index < active ? "✓" : index + 1}</i><span>{label}</span></li>)}</ol></div><footer>{Number(order.discountAmount) > 0 && <span>Chegirma −{money(order.discountAmount)}</span>}<b>Jami: {money(order.totalAmount)}</b></footer></article>; })}{!ordersLoading && !activeOrders.length && <div className="customer-orders-empty"><b>Faol buyurtma yo‘q</b><span>Yetkazilgan buyurtmalarni buyurtmalar arxivida ko‘rishingiz mumkin.</span><Link href="/profile/orders">Buyurtmalar arxivi</Link></div>}</section>
