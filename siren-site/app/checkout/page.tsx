...
function PickupPaymentSelector({ payment, onChange }: { payment: Payment; onChange: (payment: Payment) => void }) {
  ...
  return <section className="commerce-checkout__pickup-payment"><p><b>Оплата</b><span>Выберите, когда оплатить заказ.</span></p><button type="button" className={payOnPickup ? "is-selected" : ""} onClick={() => onChange("НАЛИЧНЫЕ")}><span>Оплата при получении<small>Оплатите заказ в выбранной точке самовывоза.</small></span><i><Check size={14} /></i></button><button type="button" className={!payOnPickup ? "is-selected" : ""} onClick={() => onChange("PAYME")}><span>Оплатить сейчас через Payme<small>Безопасная онлайн-оплата через Payme.</small></span><i><Check size={14} /></i></button></section>;
}
